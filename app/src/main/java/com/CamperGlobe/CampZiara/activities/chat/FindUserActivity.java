...
public class FindUserActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Button btnCreateChatRoom = findViewById(R.id.createChatRoom);
    ...
    private void createChat(){
        ...
        Boolean chatValid = false;
        for(UserObject mUserIterator : userList){
            if(mUserIterator.getSelected()){
                chatValid = true;
                newChatMap.put("users/"+ mUserIterator.getuId(), true);
                mUserDbRef.child(mUserIterator.getuId()).child("chat").child(key).setValue(true);
            }
        }
        if(chatValid){
            chatInfoDb.updateChildren(newChatMap);
            mUserDbRef.child(FirebaseAuth.getInstance().getUid()).child("chat").child(key).setValue(true);
